...
The game page is now a proper installable web app — on Android (Chrome) or iOS (Safari), open the link and use **"Add to Home Screen"** (or you'll see an automatic install prompt on Android). It'll get its own icon and open full-screen without browser address bars, just like a native app. This also lays the groundwork for turning it into a real installable `.apk` later, once the server is hosted online instead of just on the local network.
